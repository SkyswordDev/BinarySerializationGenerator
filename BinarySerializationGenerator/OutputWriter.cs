...
namespace BinarySerializationGenerator
{
    internal static class OutputWriter
    {
        ...
        private static bool ValidateAndParseObject(BuiltInTypeValidator validator, object constantSemanticValue, string argumentExpression, GeneratorExecutionContext context, Location location, out string assignExpression)
        {
            ...
            else if (constantSemanticValue is float valueFloat)
            {
                result = validator.ValidateFloat(valueFloat, context, location);
                assignExpression = argumentExpression;
            }
            else if (constantSemanticValue is double valueDouble)
            {
                result = validator.ValidateDouble(valueDouble, context, location);
                assignExpression = argumentExpression;
            }
            else if (constantSemanticValue is decimal valueDecimal)
            {
                result = validator.ValidateDecimal(valueDecimal, context, location);
                assignExpression = argumentExpression;
            }
        ...
        private static void WriteResult(GeneratorExecutionContext context, SemanticModel semanticModel, OutputWriterData data)
        {
            ...
            resultFileName.Append(data.TypeDeclaration.Identifier.Text);
            resultFileName.Append(".g");
        ...
        private static void ProcessSerializedFields(SemanticModel semanticModel, OutputWriterData data, string indent, out string constructorParams, out string constructorLines, out string serializationLines, out string deserializationLines)
        {
            ...
            foreach(SerializedFieldInfo serializedFieldInfo in data.Fields)
            {
                ...
                TypeSyntax fieldTypeSyntax = fieldDeclaration.Declaration.Type;
                string fieldTypeStr = fieldTypeSyntax.ToString();
                string readMethodName = serializedFieldInfo.BinaryReaderMethodOrNull;

                TypeInfo fieldTypeSymbol = semanticModel.GetTypeInfo(fieldDeclaration.Declaration.Type);

                data.AddDebugLine(fieldTypeSymbol.Type.ToString());
                string fullFieldType = fieldTypeSymbol.Type.ToString();
                ...
                foreach (VariableDeclaratorSyntax variable in fieldDeclaration.Declaration.Variables)
                {
                    ...
                    else
                    {
                        ...
                        {
                            AddConstructorAssignedParameter(constructorParamsBuilder, constructorLinesBuilder, indent, fullFieldType, fieldName);
                            AddFieldSerialization(serializationLinesBuilder, indent, fieldName, data.SerializationMethodName);
                            AddFieldDeserialization(deserializationLinesBuilder, deserializationMethodConstructorCallParamsBuilder, indent, fullFieldType, fieldName, data.DeserializationMethodName);
                        }
                        else
                        {
                            AddConstructorAssignedParameter(constructorParamsBuilder, constructorLinesBuilder, indent, fieldTypeStr, fieldName);
                            AddBuiltInFieldSerialization(serializationLinesBuilder, indent, fieldName);
                            AddBuiltInFieldDeserialization(deserializationLinesBuilder, deserializationMethodConstructorCallParamsBuilder, indent, fieldTypeStr, fieldName, readMethodName);
                        }
                    }
                }
            }
            ...
            deserializationMethodConstructorCallParamsBuilder.Append($"({ConstructorDuplicationAvoidanceParameterClassName})null!");
        ...
        private static void AddFieldSerialization(StringBuilder serializationLines, string indent, string fieldName, string serializationMethodName)
        {
            ...
            serializationLines.Append("(stream, maxDepth - 1);\n");
        }
        ...
        private static void AddFieldDeserialization(StringBuilder deserializationLines, StringBuilder deserializationMethodConstructorCallParams, string indent, string fullFieldType, string fieldName, string deserializationMethodName)
        {
            deserializationLines.Append(indent);
            deserializationLines.Append("if (!");
            deserializationLines.Append(fullFieldType);
            deserializationLines.Append('.');
